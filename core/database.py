...
def initialize_database():
    ...
    # THE BUG FIX (SCHEMA MIGRATION): Auto-Patch the old table into the new target_reps_min/max schema
    cursor.execute("PRAGMA table_info(routine_exercises)")
    columns = [col['name'] for col in cursor.fetchall()]
    
    if "target_reps_min" not in columns:
        print("Executing Schema Migration: Adding target_reps_min...")
    ...
    if "target_reps_max" not in columns:
        print("Executing Schema Migration: Adding target_reps_max...")
        cursor.execute("ALTER TABLE routine_exercises ADD COLUMN target_reps_max INTEGER DEFAULT 10")

    # NEW FEATURE: Program Builder & Periodization Tables
